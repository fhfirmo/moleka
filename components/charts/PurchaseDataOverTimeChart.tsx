...
const PurchaseDataOverTimeChart: React.FC<PurchaseDataOverTimeChartProps> = ({ purchaseData, theme }) => {
  const [activeMetric, setActiveMetric] = useState<MetricKey>('costValue');
  ...
  const processedData = useMemo(() => processPurchaseDataOverTime(purchaseData, activeAggregationPeriod), [purchaseData, activeAggregationPeriod]);
  ...
  const metricConfig: Record<MetricKey, { name: string; color: string; unit: string; btnColor: string }> = {
    costValue: { name: 'Custo Total', color: CHART_COLORS[3], unit: 'R$', btnColor: 'orange' },
    itemCount: { name: 'Quantidade Comprada', color: CHART_COLORS[4], unit: 'un', btnColor: 'teal' },
  };
  ...
  if (!purchaseData || purchaseData.length === 0) {
    return <p className={`text-center ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} py-10`}>Não há dados de compra para exibir.</p>;
  }
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-center items-center flex-wrap gap-x-2 gap-y-2 mb-4 text-xs">
        ...
        {/* Metric Toggles */}
        <div className="flex flex-wrap justify-center space-x-1 sm:space-x-2">
            {(['costValue', 'itemCount'] as MetricKey[]).map(metric => (
            <button
                key={metric}
...
export default PurchaseDataOverTimeChart;
